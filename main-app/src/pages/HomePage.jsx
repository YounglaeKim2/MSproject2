...
const HomePage = () => {
  ...
  const handleServiceSelect = (service) => {
    if (service === 'saju' || service === 'compatibility') {
      // 내부 라우팅으로 전환 예정
      navigate(`/${service}`)
    } else {
      // 관상학 서비스는 계속 외부 링크
    ...
    <>
      ...
      <MainContent>
        <ServiceGrid>
          ...
          <ServiceCard onClick={() => handleServiceSelect('compatibility')}>
            <ServiceIcon>💕</ServiceIcon>
            <ServiceTitle>궁합 분석</ServiceTitle>
            <ServiceDescription>
              두 사람의 사주를 비교한<br/>
              전통 궁합 분석
            </ServiceDescription>
            <ServiceFeatures>
              <Feature>• 사주 기반 궁합 분석</Feature>
              <Feature>• 오행 상성 검토</Feature>
              <Feature>• 성격 매칭 분석</Feature>
              <Feature>• 종합 궁합 점수</Feature>
            </ServiceFeatures>
          </ServiceCard>
      ...
      <Footer>
        <FooterText>
          © 2025 MSProject2. 전통 명리학과 현대 기술의 융합 서비스입니다.
        </FooterText>
        <FooterNote>
