...
echo ==========================================
echo 5 Microservices + Real-time Monitoring
...
if not exist "landing" (
    ...
    echo [INFO] Required folders: landing, SAJU, Compatibility, Physiognomy, AppService
    pause
    exit /b 1
)
...
echo [1/7] Landing Page starting... (Port 4000)
cd landing
...
echo [2/7] SAJU Backend starting... (Port 8000)
cd SAJU\backend
...
echo [3/7] SAJU Frontend starting... (Port 3000)
cd SAJU\frontend
...
echo [4/7] Mobile App starting... (Port 8082)
...
:: 5. Compatibility Service Start
echo [5/7] Compatibility Service starting... (Port 8002, 3002)
cd Compatibility\backend
pip install -r requirements.txt > ..\..\logs\compatibility-backend-install.log 2>&1
start "Compatibility Backend" cmd /c "python start_compatibility.py > ../../logs/compatibility-backend.log 2>&1"
cd ..\frontend
npm install > ..\..\logs\compatibility-frontend-install.log 2>&1
start "Compatibility Frontend" cmd /c "npm start > ../../logs/compatibility-frontend.log 2>&1"
cd ..\..
timeout /t 3 /nobreak >nul

:: 6. Physiognomy Service Start
echo [6/7] Physiognomy Service starting... (Port 8001, 3001)
...
echo [7/7] Waiting for services to start... (30 seconds)
...
echo   - Compatibility: http://localhost:8002/docs
